...
//从后台取数据，生成各省的弹窗，此时为初始化状态;
	...
	// var resumeUrl = 'http://zh.travelchina.gov.cn/sitefiles/gjly_zh/xml/tszty/tourthemetravel/tszt.json';
	var resumeUrl = 'z_map/z_json/z_tszt.json';
	var a=resumeUrl;
...
//			所有的省全部消失,各省的弹窗移除，防止加入过的的弹窗;
			...
			var tsUrl = 'http://zh.travelchina.gov.cn/sitefiles/gjly_zh/xml/objtotheme/tsztyjdgl.json';
			$.getJSON(tsUrl,function(data){
				...
				for(var i=0; i<datanum; i++){
					if(data[i].themeid==id){
		                ...
		                oA +=		'<img src="http://zh.travelchina.gov.cn/zh/public/images/index/loding.png" data-src="'+data[i].pic+'" title="'+data[i].topic+'" alt="'+data[i].topic+'" />';
		                oA +=	'</div>'
	...
	var url ='z_map/z_json/z_meijing.json';
	// var url ='http://zh.travelchina.gov.cn/sitefiles/gjly_zh/xml/meijing/meijing.json';
	$.get(url,function(data){
		...
		for(var i=0; i<datan; i++){
			...
			$('	<a href="'+"http://zh.travelchina.gov.cn"+data[i].pageurl+'.shtml" class="main_message clearfix" target="_blank" onclick="addmeijing('+data[i].id+')">'+
					'<div class="imgbox fl">'+
                    	'<img src="http://zh.travelchina.gov.cn/zh/public/images/index/loding.png" data-src="'+data[i].pic+'" title="'+data[i].bt+'" alt="'+data[i].bt+'" />'+
                    '</div>'+
...
//		防止多次点击同一个按钮;
	...
	//	右侧控制栏按钮点击后变色;
		...
		switch (oInd){
			case 0:
			$('ul.map').css('backgroundImage','url(http://zh.travelchina.gov.cn/zh/public/images/map/huadong.png)');
			var areaArr = ['p5','p6','p11','p8','p19','p14','p26'];
			fadeIn(areaArr);
			break;
			case 1:
			$('ul.map').css('backgroundImage','url(http://zh.travelchina.gov.cn/zh/public/images/map/huabei2.png)');
			var areaArr = ['p25','p2','p24','p3','p28'];
			fadeIn(areaArr);		
			break;
			case 2:
			$('ul.map').css('backgroundImage','url(http://zh.travelchina.gov.cn/zh/public/images/map/huanan.png)');
			var areaArr = ['p13','p12','p31'];
			fadeIn(areaArr);
			break;
			case 3:
			$('ul.map').css('backgroundImage','url(http://zh.travelchina.gov.cn/zh/public/images/map/huazhong.png)');
			var areaArr = ['p30','p23','p29'];
			fadeIn(areaArr);
			break;
			case 4:
			$('ul.map').css('backgroundImage','url(http://zh.travelchina.gov.cn/zh/public/images/map/dongbei2.png)');
			var areaArr = ['p1','p10','p27'];
			fadeIn(areaArr);
			break;
			case 5:
			$('ul.map').css('backgroundImage','url(http://zh.travelchina.gov.cn/zh/public/images/map/xinan.png)');
			var areaArr = ['p4','p9','p16','p17','p18'];
			fadeIn(areaArr);
			break;
			case 6:
			$('ul.map').css('backgroundImage','url(http://zh.travelchina.gov.cn/zh/public/images/map/xibei.png)');
...
//				台湾无弹窗,如点击则不动作;
		...
		dataSrc="http://zh.travelchina.gov.cn"+dataSrc;
		if(src == 'http://zh.travelchina.gov.cn/zh/public/images/index/loding.png'){
...
//	生成弹窗(页面加载完成，以及重置后的弹窗);
	 var dataUrl = 'z_map/z_json/z_province.json';
	// var dataUrl = 'http://zh.travelchina.gov.cn/sitefiles/gjly_zh/xml/tourprovince/province.json';
	...
	function createA (){
		$.getJSON(dataUrl,function(data){
			...
			for(var i = 0; i<datan; i++){
				var oA = '';
					oA +='<a href="'+"http://zh.travelchina.gov.cn"+data[i].pageurl+'.shtml" class="main_message clearfix" target="_blank">';
					oA +=   '<div class="imgbox fl">';
	                oA +=		'<img src="http://zh.travelchina.gov.cn/zh/public/images/index/loding.png" data-src="'+data[i].img+'" title="'+data[i].name+'" alt="'+data[i].name+'" />';
	                oA +=	'</div>'
	                oA +=   '<div class="fl resume_text">'
...
//重置;
	function reset(){
		...
		$('ul.map,.nanhai').show();
		$('ul.map').css('backgroundImage','url(http://zh.travelchina.gov.cn/zh/public/images/map/2.png)');
